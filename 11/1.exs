defmodule ReadMonkeyInfo do
  ...
  def set_if(line,monkey) do
    ...
    number = String.to_integer(number)
    conditions = case Map.fetch(monkey, :if) do
...
defmodule Round do
  ...
  defp inspect_item(monkey, item) do
    ...
    to_monkey = if rem(item,monkey.divisible_by) == 0 do
      monkey.if.true
    else
      monkey.if.false
    end
  ...
  defp monkey_action(monkeys, monkey_num) do
    monkey = monkeys[monkey_num]
    # IO.puts("Monkey #{monkey_num}:")
    {monkeys,total} = for item <- monkey.items, reduce: {monkeys,monkey.total} do {monkeys,total} ->
      {item,to_monkey} = inspect_item(monkey,item)
  ...
  def round(monkeys) do
    for n <- Map.keys(monkeys), reduce: monkeys do monkeys ->
      monkey_action(monkeys, n)
    end
  end
end

{monkeys,_} = File.stream!("input")
|> Stream.map(&String.trim/1)
|> Enum.reduce({%{},0}, fn line,{monkeys, current} ->
  ReadMonkeyInfo.get_monkey_info(line, monkeys, current) end)

monkeys = Enum.reduce(1..20, monkeys,fn _, monkeys -> Round.round(monkeys) end)
